...
pub enum ConfigError {
    ...
    #[error("preflight step '{0}' must be a string or inline table")] PreflightNotString(String),
    #[error("preflight step '{0}' is missing a string 'command' field")] PreflightMissingCommand(String),
    #[error("preflight timeout for '{0}' must be a positive integer")] PreflightInvalidTimeout(String),
...
pub struct Config {
    ...
    #[serde(skip)] pub preflight: Vec<(String, PreflightCommand)>,
    #[serde(skip)] pub preflight_default_timeout: Option<u64>,
...
#[derive(Debug, Clone)]
pub struct PreflightCommand { pub command: String, pub timeout: Option<u64> }
#[derive(Debug, Deserialize)]
...
impl Default for Changelog { fn default() -> Self { Self { path: None, format: default_format(), entry_template: default_entry_template() } } }
impl Default for Git { fn default() -> Self { Self { tag_prefix: default_tag_prefix(), commit_message: default_commit_message(), require_clean_tree: default_require_clean_tree(), require_branch: None } } }
...
pub fn discover(start_dir: impl AsRef<Path>) -> Result<Config, ConfigError> {
    ...
    while let Some(d) = dir { let candidate = d.join("release.toml"); if candidate.is_file() { return load(candidate); } dir = d.parent(); }
    Err(ConfigError::NotFound(start.display().to_string()))
}
fn resolve(root: &Path, path: &str) -> String { let p = Path::new(path); if p.is_absolute() { path.into() } else { root.join(p).display().to_string() } }

fn parse_preflight(text: &str) -> Result<(Vec<(String, PreflightCommand)>, Option<u64>), ConfigError> {
    ...
    if let Some(table) = doc.get("preflight").and_then(|v| v.as_table()) {
        for (k, v) in table.iter() {
            if k == "default_timeout" { default_timeout = Some(parse_timeout(k, v.as_value().ok_or_else(|| ConfigError::PreflightInvalidTimeout(k.into()))?)?); continue; }
            cmds.push((k.into(), parse_step(k, v)?));
        }
    }
    if let Some(t) = default_timeout { for (_, c) in &mut cmds { if c.timeout.is_none() { c.timeout = Some(t); } } }
    Ok((cmds, default_timeout))
}
fn parse_step(name: &str, v: &toml_edit::Item) -> Result<PreflightCommand, ConfigError> {
    if let Some(s) = v.as_str() { return Ok(PreflightCommand { command: s.into(), timeout: None }); }
    let tbl = v.as_inline_table().ok_or_else(|| ConfigError::PreflightNotString(name.into()))?;
    let cmd = tbl.get("command").and_then(|c| c.as_str()).ok_or_else(|| ConfigError::PreflightMissingCommand(name.into()))?;
    let timeout = tbl.get("timeout").map(|t| parse_timeout(name, t)).transpose()?;
    Ok(PreflightCommand { command: cmd.into(), timeout })
}
fn parse_timeout(name: &str, v: &toml_edit::Value) -> Result<u64, ConfigError> {
    let n = v.as_integer().ok_or_else(|| ConfigError::PreflightInvalidTimeout(name.into()))?;
    if n <= 0 { return Err(ConfigError::PreflightInvalidTimeout(name.into())); }
    n.try_into().map_err(|_| ConfigError::PreflightInvalidTimeout(name.into()))
}

fn validate(config: &Config) -> Result<(), ConfigError> {
    let mut seen = HashSet::new();
    for m in &config.manifest { if !seen.insert(&m.path) { return Err(ConfigError::DuplicateManifestPath(m.path.clone())); } }
    if !config.manifest.iter().any(|m| m.path == config.version.current_source) { return Err(ConfigError::CurrentSourceNotFound(config.version.current_source.clone())); }
    for m in &config.manifest {
        match m.kind.as_str() {
            "json" => require(m.field.as_ref(), m, "field")?,
            "gradle" => { require(m.version_name_field.as_ref(), m, "version_name_field")?; require(m.version_code_field.as_ref(), m, "version_code_field")?; }
            "regex" => { require(m.pattern.as_ref(), m, "pattern")?; require(m.replacement.as_ref(), m, "replacement")?; }
            "toml" | "cargo-package" => {}
...
fn require<T>(opt: Option<&T>, m: &Manifest, field: &str) -> Result<(), ConfigError> { if opt.is_none() { Err(ConfigError::MissingField(m.path.clone(), m.kind.clone(), field.into())) } else { Ok(()) } }
...
mod tests {
    ...
    fn load_str(s: &str) -> Result<Config, ConfigError> {
        ...
        c.preflight = p; c.preflight_default_timeout = d; c.root_dir = std::env::current_dir().unwrap(); validate(&c)?; Ok(c)
    }
    fn manifest(kind: &str, extra: &str) -> String { format!("[version]\ncurrent_source = \"a\"\n\n[[manifest]]\npath = \"a\"\nkind = \"{kind}\"\n{extra}") }
    #[test]
    ...
    fn preflight_timeouts() {
        let cfg = load_str(&manifest("cargo-package", r#"[preflight]
default_timeout = 5
a = "echo a"
